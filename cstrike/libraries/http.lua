...
local steam_http_raw = ffi.cast("uint32_t**", ffi.cast("char**", ffi.cast("char*", client.find_sig("client.dll", "B9 ? ? ? ? E8 ? ? ? ? 83 3D ? ? ? ? ? 0F 84")) + 1)[0] + 48)[0] or error("steam_http error")
...
callbacks.register("paint", function ()
	for _, instance in ipairs(requests) do
		if global_vars.curtime - instance.ls > instance.task_interval then
			instance:_process_tasks()
			instance.ls = global_vars.curtime
		end
	end
end)
...
function http.new(task)
	task = task or {}
	local instance = setmetatable({requests = {}, task_interval = task.task_interval or 0.3, enable_debug = task.debug or false, timeout = task.timeout or 10, ls = global_vars.curtime}, http_mt)
